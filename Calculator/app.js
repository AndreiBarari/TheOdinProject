class Calculator {
  ...
  getDisplayNumber(number) {
    ...
    const intereDigits = parseFloat(stringNumber.split(".")[0]);
    const decimalDigits = stringNumber.split(".")[1];
    let integerDisplay;
    if (isNaN(intereDigits)) {
      integerDisplay = "";
    } else {
      integerDisplay = intereDigits.toLocaleString("en", {
...
const deteletButton = document.querySelector("[data-delete]");
...
deteletButton.addEventListener("click", (button) => {
  calculator.delete();
  calculator.updateDisplay();
});
